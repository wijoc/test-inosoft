...
class MobilController extends KendaraanController
{
    ...
    public function mobilAddStock (Request $request) {
        $validator = Validator::make($request->all(), [
                'id' => 'required|exists:App\Models\Mobil,_id',
                'add_qty' => 'required|numeric|gte:0'
            ], [
                'id.required' => 'ID is required.',
                'id.exists' => 'Mobil with given id not found.',
        ...
        if ($validator->fails()) {
            ...
            $increaseStock = $this->mobilModel->increaseStock(0, ['id' => $validator->validated()['id'], 'qty' => intval($validator->validated()['add_qty'])]);
